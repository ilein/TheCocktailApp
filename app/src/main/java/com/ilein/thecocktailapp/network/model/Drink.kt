...
data class Drink (
    ...
    fun getIngredientsText(): String {
        val ingredientList = listOf(Pair(strIngredient1, strMeasure1),
        ...
        var i = 0
        val text: String = ingredientList.joinToString(separator = "") { itPair ->
            itPair.first?.let {
                i++
                return@let "${i}. $it ${itPair.second?.let {"- $it"} ?: ""}\n"
            } ?: ""
        }
        return text
    }
